...
Some of these functions will be also used for integrating moments.LD and dadi,
    ...
    if any_fn:
        for pop_nu in nus:
            if hasattr(pop_nu, "__len__"):
                ...
                nu.append(
                    lambda t, nu0=nu0, growth_rate=growth_rate: nu0
...
def add_size_to_nus(G, pop, time_left):
    ...
    if "nu" in G.nodes[pop]:
        ...
        if "nu0" in G.nodes[pop] and "nuF" in G.nodes[pop]:
            growth_rate = (
                np.log(G.nodes[pop]["nuF"] / G.nodes[pop]["nu0"]) / G.nodes[pop]["T"]
            )
            nu0 = G.nodes[pop]["nu0"] * np.exp(growth_rate * tt)
            return [nu0, growth_rate]
        elif "growth_rate" in G.nodes[pop] and "nuF" in G.nodes[pop]:
            nu0_pop = G.nodes[pop]["nuF"] * np.exp(
                -G.nodes[pop]["growth_rate"] * G.nodes[pop]["T"]
            )
            ...
            if this_pop in dg.successors:
                ...
                for child in dg.successors[this_pop]:
                    if child not in new_pops:
                        ...
                        new_times += [
                            dg.G.nodes[child]["T"] for child in dg.successors[this_pop]
                        ]
                        new_nus += [
                            add_size_to_nus(dg.G, child, dg.G.nodes[child]["T"])
                            for child in dg.successors[this_pop]
                        ]
                if len(dg.successors[this_pop]) >= 2:
                    ...
                    if dg.predecessors[child] == [this_pop]:
                        ...
                        if event_recorded == 1:
                            ...
                            weights = (
                                dg.G.get_edge_data(parent1, child)["weight"],
                                dg.G.get_edge_data(parent2, child)["weight"],
                            )
                            new_events.append(
                                ("merger", (parent1, parent2), weights, child)
                            )
        ...
        if this_pop in pulse_migration_events:
            for pulse_event in pulse_migration_events[this_pop]:
                if pulse_event[0] < 0:  # this pulse already occurred
                    ...
                    new_events.append(
                        ("pulse", this_pop, pulse_event[1], pulse_event[2])
                    )
...
def get_moments_arguments(dg):
    ...
    while advance == True:
        ...
        if np.all(np.array(time_left) < tol) and np.all(
            [p not in dg.successors for p in present_pops[-1]]
        ):
            advance = False
        else:
            new_pops, new_times, new_nus, new_events = get_next_events(
                dg, pulse_migration_events, time_left, present_pops
            )
            ...
            time_left = [
                pop_time_left - t_epoch if pop_time_left - t_epoch >= tol else 0
                for pop_time_left in time_left
            ]
    ...
    assert np.all(
        [t >= 0 for t in integration_times]
    ), "negative integration time -- probably some mistake in defining the demography"
    ...
    if np.all([abs(accumulated_times[pop] - max_time) < tol for pop in sampled_pops]):
        ...
        for pop in sampled_pops:
            ...
            if abs(accumulated_times[pop] - max_time) < tol:
                ...
                G.add_node(pop, nu=G.nodes[pop_pre]["nu"], T=time_left, frozen=True)
...
def evolve_ld(dg, rho=None, theta=None, pop_ids=None, augment=True):
    ...
    for ii, (pops, T, nu, mig_mat, frozen, selfing) in enumerate(
        zip(
            present_pops,
            integration_times,
            nus,
        ...
        Y.integrate(
            nu_epoch, T, rho=rho, theta=theta, m=mig_mat, selfing=selfing, frozen=frozen
        )

        # apply events
        if ii < len(
            events
        ):  ## want to change this to allow events at very end of simulation
...
def ld_marginalize_nonpresent(Y, pop_ids):
    ...
    marge_indexes = [Y.pop_ids.index(pop) for pop in pops_to_marginalize]
...
def ld_merge(Y, pops_to_merge, weights, pop_to):
    ...
    if pop1_ind < pop2_ind:
        Y = Y.merge(pop1_ind, pop2_ind, weights[0])
    else:
        Y = Y.merge(pop2_ind, pop1_ind, weights[1])
    Y.pop_ids = ids_to
    return Y
...
def ld_pulse(Y, pop_from, pop_to, pulse_weight):
    ...
    if pop_to in Y.pop_ids:
        ...
        Y = Y.pulse_migrate(ind_from, ind_to, pulse_weight)
    else:
        print(
            f"warning: pop_to ({pop_to}) in pulse_migrate isn't in present pops {Y.pop_ids}"
        )
    return Y
...
def moments_fs_root_equilibrium(
    ns0, nu, theta, pop_id, gamma=None, h=0.5, reversible=False
):
    if reversible is False:
        if gamma is not None:
            ss = moments.LinearSystem_1D.steady_state_1D(
                ns0, gamma=gamma, h=h, theta=theta
            )
        else:
        ...
        if gamma is None:
            gamma = 0.0
        ss = moments.LinearSystem_1D.steady_state_1D_reversible(
            ns0, gamma=gamma, theta_fd=theta, theta_bd=theta
        )
...
def check_max_num_pops(present_pops, num):
    for pps in present_pops:
        assert (
            len(pps) <= num
        ), """to run this method, we can't have more \
...
def evolve_sfs_moments(
    ...
    num_lineages = get_number_needed_lineages(dg_sim, pop_ids, sample_sizes, events)
    # initialize the sfs at the root of the demography
    fs = moments_fs_root_equilibrium(
        num_lineages[dg_sim.root],
        nus[0][0],
        theta,
        dg_sim.root,
        gamma=gamma,
    ...
    # step through the list of integrations and events
    for ii, (pops, T, nu, mig_mat, frozen) in enumerate(
        zip(present_pops, integration_times, nus, migration_matrices, frozen_pops)
    ):
        # first get the nu_function for this epoch
        nu_epoch = get_pop_size_function(nu)
        ...
        # apply events
        if ii < len(events):
            fs = moments_apply_events(
                fs, events[ii], present_pops[ii + 1], num_lineages
            )
    ...
    if len(epoch_events) > 0:
        for e in epoch_events:
            if e[0] == "pass":
                ...
                fs = moments_merge(
                    fs, e[1], e[2], e[3], lineages
                )  # pops_from, weights, pop_to
            elif e[0] == "pulse":
...
def moments_split_2_way(fs, parent, child1, child2, lineages):
    ...
    if data.ndim == 1:
        ...
        if ids_from[0] == parent:
            fs_to = moments.Manips.split_2D_to_3D_1(
                data, lineages[child1], lineages[child2]
            )
        else:
            fs_to = moments.Manips.split_2D_to_3D_2(
                data, lineages[child1], lineages[child2]
            )
    elif data.ndim == 3:
        if ids_from[0] == parent:
            fs_to = moments.Manips.split_3D_to_4D_1(
                data, lineages[child1], lineages[child2]
            )
        elif ids_from[1] == parent:
            fs_to = moments.Manips.split_3D_to_4D_2(
                data, lineages[child1], lineages[child2]
            )
        elif ids_from[2] == parent:
            fs_to = moments.Manips.split_3D_to_4D_3(
                data, lineages[child1], lineages[child2]
            )
    elif data.ndim == 4:
        if ids_from[0] == parent:
            fs_to = moments.Manips.split_4D_to_5D_1(
                data, lineages[child1], lineages[child2]
            )
        elif ids_from[1] == parent:
            fs_to = moments.Manips.split_4D_to_5D_2(
                data, lineages[child1], lineages[child2]
            )
        elif ids_from[2] == parent:
            fs_to = moments.Manips.split_4D_to_5D_3(
                data, lineages[child1], lineages[child2]
            )
        elif ids_from[3] == parent:
            fs_to = moments.Manips.split_4D_to_5D_4(
                data, lineages[child1], lineages[child2]
            )
...
def moments_merge(fs, pops_to_merge, weights, pop_to, lineages):
    ...
    # use admix_into_new
    if pop1_ind < pop2_ind:
        data = moments.Manips.admix_into_new(
            data, pop1_ind, pop2_ind, lineages[pop_to], weights[0]
        )
    else:
        data = moments.Manips.admix_into_new(
            data, pop2_ind, pop1_ind, lineages[pop_to], weights[1]
        )
    data.pop_ids = ids_to
    return data
...
def moments_pulse(fs, pop_from, pop_to, pulse_weight):
    ...
    if pop_to in fs.pop_ids:
        ...
        fs = moments.Manips.admix_inplace(
            fs, ind_from, ind_to, n_from_post, pulse_weight
        )
    else:
        print(
            f"warning: pop_to ({pop_to}) in pulse_migrate isn't in present pops {fs.pop_ids}"
        )
    return fs
...
def evolve_sfs_dadi(
    ...
    phi = dadi_root_equilibrium(pts, grid, nu=nus[0][0], theta=theta, gamma=gamma, h=h)
    ...
    for ii, (pops, T, nu, mig_mat, frozen) in enumerate(
        ...
        # question: does
        if ii < len(events):
            phi = dadi_apply_events(
                phi, grid, pts, events[ii], present_pops[ii], present_pops[ii + 1]
            )
...
def dadi_root_equilibrium(pts, xx, nu=1.0, theta=1.0, gamma=0.0, h=0.5):
    if hasattr(pts, "__len__"):
        phi = [
            dadi.PhiManip.phi_1D(x, nu=nu, theta0=theta, gamma=gamma, h=h) for x in xx
        ]
    else:
...
def integrate_dadi(this_phi, this_xx, T, nu, m, gamma, h, theta, frozen):
    if this_phi.ndim == 1:
        this_phi = dadi.Integration.one_pop(
            this_phi,
            this_xx,
            T,
            nu=nu[0],
        ...
        this_phi = dadi.Integration.two_pops(
            this_phi,
            this_xx,
            T,
            nu1=nu[0],
        ...
        this_phi = dadi.Integration.three_pops(
            this_phi,
            this_xx,
            T,
            nu1=nu[0],
...
def integrate_phis(phi, xx, pts, nu, T, theta=1.0, m=None, frozen=None, gamma=0, h=0.5):
    if hasattr(pts, "__len__"):
        for ii, (this_phi, this_xx) in enumerate(zip(phi, xx)):
            phi[ii] = integrate_dadi(
                this_phi, this_xx, T, nu, m, gamma, h, theta, frozen
            )
    else:
...
def sample_dadi(phi, grid, pts, ns):
    if hasattr(pts, "__len__"):
        spectra = [
            dadi.Spectrum.from_phi(this_phi, ns, tuple([this_grid] * this_phi.ndim))
...
def dadi_apply_events(
    phi, grid, pts, epoch_events, prev_present_pops, next_present_pops
):
    """
    takes the LDstats object and applied events (such as splits, mergers,
    pulse migrations, and marginalizations)
    ...
    if len(epoch_events) > 0:
        for e in epoch_events:
            if e[0] == "pass":
                phi = dadi_pass(
                    phi, pts, e[1], e[2], prev_present_pops, next_present_pops
                )
            elif e[0] == "split":
                phi = dadi_split(
                    phi,
                ...
                phi = dadi_marginalize(
                    phi, grid, pts, e[1], prev_present_pops, next_present_pops
                )
    return phi


def dadi_marginalize(
    phi, grid, pts, pop_to_remove, prev_present_pops, next_present_pops
):
    ...
    if hasattr(pts, "__len__"):
        phi = [
            dadi.PhiManip.remove_pop(this_phi, this_grid, index_to_remove + 1)
            for (this_phi, this_grid) in zip(phi, grid)
        ]
    else:
...
def dadi_split(
    phi, grid, pts, parent, child1, child2, prev_present_pops, next_present_pops
):
    if hasattr(pts, "__len__"):
        if phi[0].ndim == 1:
            phi = [
                dadi.PhiManip.phi_1D_to_2D(this_grid, this_phi)
            ...
            if prev_present_pops[0] == parent:
                phi = [
                    dadi.PhiManip.phi_2D_to_3D_split_1(this_grid, this_phi)
                    for this_grid, this_phi in zip(grid, phi)
                ]
            else:
                phi = [
                    dadi.PhiManip.phi_2D_to_3D_split_2(this_grid, this_phi)
                    for this_grid, this_phi in zip(grid, phi)
                ]
...
def dadi_merge(
    phi, grid, pts, pops_to_merge, weights, pop_to, prev_present_pops, next_present_pops
):
    """
    Two populations (pops_to_merge = [popA, popB]) merge (with given weights)
    and form new population (pop_to).
    ...
    if len(prev_present_pops) == 2:
        ...
        if hasattr(pts, "__len__"):
            for ii, (this_phi, this_grid) in enumerate(zip(phi, grid)):
                this_phi = dadi.PhiManip.phi_2D_to_3D_admix(
                    this_phi, f, this_grid, this_grid, this_grid
                )
                this_phi = dadi.PhiManip.remove_pop(this_phi, this_grid, 1)
                this_phi = dadi.PhiManip.remove_pop(this_phi, this_grid, 1)
                phi[ii] = this_phi
        else:
            phi = dadi.PhiManip.phi_2D_to_3D_admix(phi, f, grid, grid, grid)
        ...
        phi = dadi_pulse(
            phi, grid, pts, pop2, pop1, weights[1], prev_present_pops, prev_present_pops
        )
        ...
        # marginalize
        phi = dadi_marginalize(phi, grid, pts, pop2, prev_present_pops, new_ids)
...
def dadi_pulse(
    phi, grid, pts, pop_from, pop_to, pulse_weight, prev_present_pops, next_present_pops
):
    """
    A pulse migration event
    Different from merger, where the two parental populations are replaced by
        ...
        if len(prev_present_pops) == 2:
            if ind_from == 0 and ind_to == 1:
                ...
                if hasattr(pts, "__len__"):
                    phi = [
                        dadi.PhiManip.phi_2D_admix_1_into_2(
                            this_phi, pulse_weight, this_grid, this_grid
                        )
                        for this_phi, this_grid in zip(phi, grid)
                    ]
                else:
                    phi = dadi.PhiManip.phi_2D_admix_1_into_2(
                        phi, pulse_weight, grid, grid
                    )
                ...
                if hasattr(pts, "__len__"):
                    ...
                    phi = dadi.PhiManip.phi_2D_admix_2_into_1(
                        phi, pulse_weight, grid, grid
                    )
        elif len(prev_present_pops) == 3:
            if ind_from == 0 and ind_to == 1:
                ...
                if hasattr(pts, "__len__"):
                    phi = [
                        dadi.PhiManip.phi_3D_admix_1_and_3_into_2(
                            this_phi, pulse_weight, 0, this_grid, this_grid, this_grid
                        )
                        for this_phi, this_grid in zip(phi, grid)
                    ]
                else:
                    phi = dadi.PhiManip.phi_3D_admix_1_and_3_into_2(
                        phi, pulse_weight, 0, grid, grid, grid
                    )
                ...
                if hasattr(pts, "__len__"):
                    ...
                    phi = dadi.PhiManip.phi_3D_admix_1_and_2_into_3(
                        phi, pulse_weight, 0, grid, grid, grid
                    )
                ...
                if hasattr(pts, "__len__"):
                    ...
                    phi = dadi.PhiManip.phi_3D_admix_2_and_3_into_1(
                        phi, pulse_weight, 0, grid, grid, grid
                    )
                ...
                if hasattr(pts, "__len__"):
                    phi = [
                        dadi.PhiManip.phi_3D_admix_1_and_2_into_3(
                            this_phi, 0, pulse_weight, this_grid, this_grid, this_grid
                        )
                        for this_phi, this_grid in zip(phi, grid)
                    ]
                else:
                    phi = dadi.PhiManip.phi_3D_admix_1_and_2_into_3(
                        phi, 0, pulse_weight, grid, grid, grid
                    )
