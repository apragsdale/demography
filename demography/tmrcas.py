...
def compute_tmrcas(dg, pop_ids, Ne, order, selfing):
    ...
    if any_selfing:
        T = integrate_tmrca_selfing(dg, Ne, order, pop_ids, selfing)
    else:
...
def apply_events(Tmrcas, events, current_pop_ids, next_pop_ids, order, selfing=False):
    for event in events:
        if event[0] == 'split':
    ...
    Tmrcas, current_pop_ids = reorder_pops(Tmrcas, order, current_pop_ids,
                                           next_pop_ids, selfing=selfing)
    assert np.all([id1 == id2 for id1, id2 in zip(current_pop_ids, next_pop_ids)])
    return Tmrcas, current_pop_ids


def split(Tmrcas, pop_from, pops_to, pop_ids, order, selfing=False):
    if selfing == True:
        old_names = tmrca_vector_selfing(order, len(pop_ids))
        new_names = tmrca_vector_selfing(order, len(pop_ids)+1)
    else:
...
def reorder_pops(Tmrcas, order, old_pop_ids, new_pop_ids, selfing=False):
    if selfing == True:
        names = tmrca_vector_selfing(order, len(old_pop_ids))
    else:
...
def marginalize(Tmrcas, pop_out, pop_ids, order, selfing=False):
    if selfing == True:
        old_names = tmrca_vector_selfing(order, len(pop_ids))
    else:
        old_names = tmrca_vector(order, len(pop_ids))
...
def marginalize_nonpresent(Tmrcas, pop_ids, current_pop_ids, order, selfing=False):
...
def pulse_migrate(Tmrcas, pop_from, pop_to, f, current_pop_ids, order, selfing=False):
    num_pops = len(current_pop_ids)
    idx_from = current_pop_ids.index(pop_from)
    idx_to = current_pop_ids.index(pop_to)
    Pulse = pulse_matrix(num_pops, order, idx_from, idx_to, f, selfing=selfing)
    Tmrcas = Pulse.dot(Tmrcas)
    return Tmrcas, current_pop_ids


def pulse_matrix(num_pops, order, idx_from, idx_to, f, selfing=False):
    if selfing == True:
        names = tmrca_vector_selfing(order, num_pops)
    else:
    ...
    for i, mom in enumerate(names):
        ...
        p1 = int(mom.split('_')[1])
        p2 = int(mom.split('_')[2])
        if x.startswith('T'): # between individual TMRCA
            if p1 == p2 == idx_to:
                ...
                P[i, i] = 1
        elif x.startswith('W'): # within individual TMRCA
            assert p1 == p2, "cannot have within individual TMRCA from multiple demes"
            if p1 == idx_to:
                mom_from = rename_moment('_'.join([x, str(idx_from), str(idx_from)]))
                P[i, names.index(mom_from)] = f
                P[i, names.index(mom)] = (1-f)
...
def tmrca_vector_selfing(order, num_pops):
    """
    [W_{1,1}^n, W_{2,2}^n, ..., W_{p,p}^n,
        T_{1,1}^n, T_{1,2}^n, ..., T_{p,p}^n, 
        W_{1,1}^{n-1}, ... 
        T_{1,1}^{n-1}, ...]
    """
    tmrcas = []
    ...
    for i in range(1, order+1)[::-1]: # from largest to smallest order
        for j in range(num_pops):
            tmrcas.append(f'W{i}_{j}_{j}')
        for pair in pairs:
            tmrcas.append(f'T{i}_{pair[0]}_{pair[1]}')
    return tmrcas


def add_selfing_rates(dg, pself):
    """
    for each population in the demography object, we add the default selfing
    rate if it does not have a given selfing rate
    """
    dg_self = copy.deepcopy(dg)
    for p in dg.G.nodes:
        if 'selfing' not in dg.G.nodes[p]:
            dg_self.G.nodes[p]['selfing'] = pself
    return dg_self
...
def integrate_tmrca_selfing(dg, Ne, order, pop_ids, pself):
    ...
    # get events
    (present_pops, integration_times, nus, migration_matrices, frozen_pops,
        selfing_rates, events) = integration.get_moments_arguments(dg_self)
    ...
    for ii, (pops, T, nu, mig_mat, frozen, selfing) in enumerate(zip(
        ...
        # evolve over generations in this epoch
        Tmrcas = evolve_t_selfing(Tmrcas, order, Ns, mig_mat, frozen,
                                  selfing)

        # apply events (splits, mergers, pulses, marginalizations)
        ### question: do any of the events require passing selfing rate?
        ### such as pulse admixture?
        if ii < len(events):
            Tmrcas, current_pop_ids = apply_events(Tmrcas, events[ii],
                                                   current_pop_ids,
                                                   present_pops[ii+1], order,
    ...
    Tmrcas, current_pop_ids = reorder_pops(Tmrcas, order, current_pop_ids,
                                           pop_ids, selfing=True)

    return Tmrcas


def steady_state_tmrca_selfing(dg, Ne, order, pself):
    """
    Get the steady state T = -inv(P).dot(ones)
    returns [W_n, T_n, W_n-1, T_(n-1), ...]
    """
    P = transition_selfing(order, [Ne], [[1]], [False], [pself])
    return np.linalg.inv(np.eye(2*order) - P).dot(np.ones(2*order))


def transition_selfing(order, N, m, frozen, pself):
    """
    Moment vector has order (W_{1,1}^n, W_{2,2}^n, ..., T_{1,1}^n, T_{1,2}^n, ...)
    """
    num_pops = len(N)
    for i,freeze in enumerate(frozen):
        if freeze is True:
            N[i] = np.inf
            pself[i] = 0
    tmrcas = tmrca_vector_selfing(order, num_pops)
    pairs = list(combinations_with_replacement(range(num_pops), 2))
    P = np.zeros((order*(num_pops+len(pairs)), order*(num_pops+len(pairs))))
    # within diploid transitions
        ...
        for n in range(1, order+1)[::-1]:
            ...
            for (jj,kk) in pairs:
                    ...
                    for kappa in range(1, n+1):
                        ...
                        P[this_idx, tmrcas.index(mom_from)] += fac * (1-f) * m_ij * m_ik * _choose(n, kappa)

    # between diploid transitions
    for (ii,jj) in pairs:
        for n in range(1, order+1)[::-1]: # order of moment for given pair
            this_idx = tmrcas.index(f'T{n}_{ii}_{jj}')
            if ii == jj: #T^n_{ii,ii}
                for kk in range(num_pops):
                    m_ik = m[ii][kk]
                    for kappa in range(1, n+1):
                        mom_from = f'W{kappa}_{kk}_{kk}'
                        P[this_idx, tmrcas.index(mom_from)] += m_ik / 2 / N[ii] * _choose(n, kappa)
                for (kk,ll) in pairs:
                    m_ik = m[ii][kk]
                    m_il = m[ii][ll]
                    fac = 1+(kk!=ll)
                    for kappa in range(1, n+1):
                        mom_from = f'T{kappa}_{kk}_{ll}'
                        P[this_idx, tmrcas.index(mom_from)] += fac * m_ik * m_il * (1-1/N[ii]) * _choose(n, kappa)
            else:
                for kk in range(num_pops):
                    for ll in range(num_pops):
                        ...
                        m_jl = m[jj][ll]
                        for kappa in range(1, n+1):
                            mom_from = rename_moment(f'T{kappa}_{kk}_{ll}')
                            P[this_idx, tmrcas.index(mom_from)] += m_ik * m_jl * _choose(n, kappa)
    return P


def evolve_t_selfing(Tmrcas, order, Ns, mig_mat, frozen, pself):
    if len(Ns[0]) == 0:
        return Tmrcas
    else:
        # if diagonals of mig_mat are zero and row doesn't sum to one, set diags
        ...
        for i in range(len(Ns[0])):
            current_Ns = [Ns[j][i] for j in range(len(Ns))]
            if not np.all(current_Ns == last_Ns): 
                P = transition_selfing(order, current_Ns, mig_mat, frozen, pself)
            Tmrcas = P.dot(Tmrcas) + 1
        return Tmrcas
